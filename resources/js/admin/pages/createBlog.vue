				...
				<div class="_1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20">
                    ...
                    <div class="_input_field">
                        ...
                        <Select filterable multiple placeholder="Select tag" v-model="data.tag_id">
                            <Option v-for="(t) in tags" :value="t.id" :key="t.id">{{ t.tagName }}</Option>
                        </Select>  
                    </div>
					<div class="_input_field">
...
export default {
	...
	async created() {
        const [cat, tagsResponse] = await Promise.all([
            this.callApi('get', 'app/get-category'),
            this.callApi('get', 'app/get-tags'),
        ]);
        if (cat.status == 200) {
            this.category = cat.data;
            this.tags = tagsResponse.data; // tagsResponse.data ni ishlating
        } else {
            this.swr();
        }
    }
